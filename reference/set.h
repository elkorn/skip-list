    ...
    TAllocator > {
        ...
        template < typename TNodePtr, typename TPtr, typename TRef >
            class node_iterator {
                ...
                explicit node_iterator(TNodePtr node, set * set_): m_node(node),
                m_set(set_) { /**/ }
                template < typename UNodePtr,
                typename UPtr,
                typename URef >
                    node_iterator(const node_iterator < UNodePtr, UPtr, URef > & rhs): m_node(rhs.node()),
                m_set(rhs.get_set()) {
                    /**/
                }
                ...
                private: TNodePtr find_next_node(TNodePtr node) const {
                    return 0;
                }
        ...
        explicit set(const allocator_type & allocator = allocator_type()): Base(allocator) {}
